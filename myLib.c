...
void drawRect4(int col, int row, int width, int height, volatile unsigned char colorIndex) {
    ...
    if (!(col & 1) && !(width & 1)) {
        for (int r0 = 0; r0 < height; r0++) {
            DMANow(3, &pixelData, &videoBuffer[OFFSET(col, row+r0, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | width / 2);
        }
    ...
    //even col odd width
        ...
        for (int r1 = 0; r1 < height; r1++) {
            if (width > 1) {
                DMANow(3, &pixelData, &videoBuffer[OFFSET(col, row+r1, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | (width - 1) / 2);
            }
            setPixel4(col + width - 1, row + r1, colorIndex);
        }
    ...
    //odd col odd width
        ...
        for (int r2 = 0; r2 < height; r2++) {
            if (width > 1) {
                DMANow(3, &pixelData, &videoBuffer[OFFSET(col + 1, row + r2, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | (width - 1) / 2);
            }
            setPixel4(col, row + r2, colorIndex);
        }
    //odd col even width
    } else {
        for (int r3 = 0; r3 < height; r3++) {
            if (width > 2) {
                DMANow(3, &pixelData, &videoBuffer[OFFSET(col + 1, row + r3, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | (width - 2) / 2);
            }
            setPixel4(col, row + r3, colorIndex);
            setPixel4(col + width - 2, row + r3, colorIndex);
        }
    }
}
